interface HeaderProps {
  ...
  onCartClick: () => void;
  onCategoryClick: (category: 'all' | 'streaming' | 'iptv') => void;
  whatsappNumber: string;
}

export function Header({ cartItemsCount, onCartClick, onCategoryClick, whatsappNumber }: HeaderProps) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a href="#products" onClick={() => onCategoryClick('streaming')} className="text-gray-400 hover:text-yellow-400 transition-colors text-sm font-medium">
              Streaming
            </a>
            <a href="#products" onClick={() => onCategoryClick('iptv')} className="text-gray-400 hover:text-yellow-400 transition-colors text-sm font-medium">
              IPTV
            </a>
            <a href="#products" onClick={() => onCategoryClick('all')} className="text-gray-400 hover:text-yellow-400 transition-colors text-sm font-medium">
              Todos
            </a>
            <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-gray-400 hover:text-green-500 transition-colors text-sm font-medium">
          ...
          <button
            onClick={onCartClick}
            aria-label={`Abrir carrito, ${cartItemsCount} artículos`}
